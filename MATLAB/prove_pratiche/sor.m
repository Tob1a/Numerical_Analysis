...
% soluzione.
%Il calcolo viene effettuato SENZA CALCOLO ESPLICITO E MEMORIZZAZIONE della matrice di iterazione, ma usando semplicemente gli elementi di A.
%SYNOPSIS:
...
while ( ~arresto )
    ...
    arresto = ( norm(xtemp - x, inf) < tol*norm(x, inf) ) || ( k == maxit ); end
    if ( k == maxit )
